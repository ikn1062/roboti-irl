...
TEST_CASE("iLQR Controller Objective Function", "[Controller]")
{
  ...
  std::pair<arma::mat, arma::mat> trajectory = createTrajectory<CartPole>(cartpole, dt);
  arma::mat X = trajectory.first;
  arma::mat U = trajectory.second;

  SimpleController controller = SimpleController(cartpole, Q, R, P, r, 4000, alpha, beta, eps);
  double obj_cost = controller.objectiveJ(X, U);
  REQUIRE(almost_equal(obj_cost, 0.0, 1e-6));

  arma::vec x0_new({0.0, 0.0, PI, 0.0});
  arma::mat U_new(U.n_rows, U.n_cols, arma::fill::zeros);
  arma::mat X_new = createTrajectory<CartPole>(cartpole, x0_new, U_new, dt);

  // FINISH THIS TEST BASED OFF XNEW
  double obj_cost_new = controller.objectiveJ(X_new, U_new);
...
TEST_CASE("iLQR Controller Calculate aT", "[Controller]")
{
  ...
  aT_test.row(1) = {0.0105, 0.0399, -30.7533, 0.1968};

  for (unsigned int i = 0; i < aT_test.n_rows; i++) {
    for (unsigned int j = 0; j < aT_test.n_cols; j++) {
