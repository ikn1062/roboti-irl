...
TEST_CASE("Cartpole Dynamics", "[CartPole]")
{
    ...
    arma::mat A = cartpole.getA(x0, u0);
    arma::mat B = cartpole.getB(x0, u0);

    A.print("A: ");
    B.print("B: ");

    arma::vec xdot_test({2.0, 0.2164155, 3.0, 0.5671797});

    xdot.print("xdot: ");
    xdot_test.print("xdot_test: ");
    for (int i = 0; i < 4; i++){
...
TEST_CASE("Cartpole Integrate Step", "[CartPole]")
{
    ...
    arma::vec xdot_test({2.0, 0.2164155, 3.0, 0.5671797});
    arma::vec x1_test = x0 + 0.5 * xdot_test;

    x1.print("x1: ");
    x1_test.print("x1_test: ");
    for (int i = 0; i < 4; i++){
        REQUIRE(ergodiclib::almost_equal(x1(i), x1_test(i), 1e-4));
    }

    std::cout << "Test Part Complete" << std::endl;
}
