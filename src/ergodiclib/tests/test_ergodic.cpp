...
TEST_CASE("Test calculateFk - Fourier Basis", "[fourierBasis]")
{
  ...
  double hk2 = Basis_2D.calculateHk(K_vec2, 1);
  assert(hk2);
  ...
  double hk4 = Basis_4D.calculateHk(K_vec3, 0);
  assert(hk4);

  double fk4 = Basis_4D.calculateFk(x3, K_vec3, 0);
  ...
  double hk5 = Basis_4D.calculateHk(K_vec4, 1);
  assert(hk5);
