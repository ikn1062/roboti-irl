...
public:
  ...
  double calculateFk(
    const arma::vec & x_i_trajectory, const std::vector<int> & K_vec,
    const int & k_idx);

  /// \brief WRITE COMMENT
  arma::rowvec calculateDFk(const arma::colvec & xt, const std::vector<int> & K_vec, const int &k_idx) const;
