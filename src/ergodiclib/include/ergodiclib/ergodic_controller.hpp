...
public:
  ...
  : BaseController<ModelTemplate>::BaseController(model_agent, Q, R, P, r, max_iter_in, a, b, e),
    q(q_val)
  {
    ergodicMeasure = new ErgodicMeasure(
      demonstrations, demo_posneg, demo_weights, model_agent.dt,
      L_dim, num_dim, K_coeff);
    tf = model_agent.tf;
  }
...
template<class ModelTemplate>
arma::vec ergController<ModelTemplate>::get_z(
  const std::vector<arma::mat> & Plist,
  const std::vector<arma::mat> & rlist) const
{
  UNUSED(rlist);
